...
int map_check(void)
{
    ...
    if(count_I!=1||count_E!=1) {abort_map();return 0;}//出入口是否唯一
    if(entrance_loc.x!=0&&entrance_loc.y!=0&&entrance_loc.x!=w-1&&entrance_loc.y!=h-1) {abort_map();return 0;}//入口是否在边界
    if(exit_loc.x!=0&&exit_loc.y!=0&&exit_loc.x!=w-1&&exit_loc.y!=h-1) {abort_map();return 0;}//出口是否在边界
     ...
     for(i=0; i<h; i++)
    {
        for(j=0; j<w; j++)
        {
            ...
            {
                if(data_st.pmap[i][j+1]==X) {count_X ++;parkcount ++;parknode[parkcount-1].I_loc.y=i;parknode[parkcount-1].I_loc.x=j+1;parknode[parkcount-1].loc.y=i;parknode[parkcount-1].loc.x=j;map_data();}
                if(data_st.pmap[i+1][j]==X) {count_X ++;parkcount ++;parknode[parkcount-1].I_loc.y=i+1;parknode[parkcount-1].I_loc.x=j;parknode[parkcount-1].loc.y=i;parknode[parkcount-1].loc.x=j;map_data();}
                if(count_X!=1) {abort_map();return 0;}
                }
...
{
    int m=parkcount-1;
    pathfind(&entrance_loc,&(parknode[m].I_loc),parknode[m].I_Path,&(parknode[m].I_len));
    pathfind(&(parknode[m].I_loc),&exit_loc,parknode[m].E_Path,&(parknode[m].E_len));
    return 1;
}
